...
export default function Rinnovo() {
  ...
  const meseGiorno = oggi.getMonth() * 100 + oggi.getDate() // es. gennaio=0, 1 gen = 1
  const inizioFinestra = new Date(anno, 0, 1)   // 1 gennaio
  const fineFinestra = new Date(anno, 2, 31)     // 31 marzo
  const finestreAperta = oggi >= inizioFinestra && oggi <= fineFinestra

  // Ha già rinnovato quest'anno?
  const haGiaRinnovato = group?.scadenza && new Date(group.scadenza).getFullYear() === anno

  // Giorni alla prossima finestra (1 gennaio anno prossimo)
  ...
  const labelStyle = { display: 'block' as const, fontSize: 13, color: '#888', marginBottom: 3 }
  const lockedStyle = {
    padding: '9px 12px', background: '#f5f5f3', border: '0.5px solid #e5e5e5',
    borderRadius: 8, fontSize: 14, color: '#888', width: '100%',
  }
  const sectionTitle = {
  ...
  return (
    <main style={{ minHeight: '100vh', background: '#f9f9f7', padding: '1.5rem' }}>
      <div style={{ maxWidth: 600, margin: '0 auto' }}>
        ...
        {/* Già rinnovato quest'anno */}
        {haGiaRinnovato && !sent && (
          <div style={{ background: '#EAF3DE', border: '0.5px solid #C0DD97', borderRadius: 12, padding: '1.5rem', fontSize: 14, color: '#3B6D11' }}>
            <div style={{ fontWeight: 500, marginBottom: 8, fontSize: 15 }}>✓ Rinnovo {anno} già effettuato</div>
            <p>Il tuo gruppo ha già rinnovato per l'anno {anno}. Il prossimo periodo di rinnovo aprirà il <strong>1 gennaio {anno + 1}</strong> e si chiuderà il <strong>31 marzo {anno + 1}</strong>.</p>
            <div style={{ marginTop: '1rem', padding: '0.7rem 1rem', background: '#d4edda', borderRadius: 8, fontSize: 13 }}>
              Mancano <strong>{giorniAllaProssima} giorni</strong> alla prossima finestra di rinnovo.
            </div>
          </div>
        )}

        {/* Fuori dalla finestra di rinnovo */}
        {!finestreAperta && !haGiaRinnovato && !sent && (
          <div style={{ background: '#EAF3DE', border: '0.5px solid #C0DD97', borderRadius: 12, padding: '1.5rem', fontSize: 14, color: '#3B6D11' }}>
            <div style={{ fontWeight: 500, marginBottom: 8, fontSize: 15 }}>✓ La tua registrazione è attiva</div>
            <p style={{ marginBottom: 8 }}>Il periodo di rinnovo annuale è <strong>dal 1 gennaio al 31 marzo</strong> di ogni anno.</p>
            <p>Il prossimo periodo di rinnovo aprirà il <strong>1 gennaio {anno + 1}</strong>.</p>
            <div style={{ marginTop: '1rem', padding: '0.7rem 1rem', background: '#d4edda', borderRadius: 8, fontSize: 13 }}>
              Mancano <strong>{giorniAllaProssima} giorni</strong> alla prossima finestra di rinnovo.
            </div>
          </div>
        ...
        {/* Finestra aperta e non ancora rinnovato */}
        {finestreAperta && !haGiaRinnovato && !sent && (
          <>
            <div style={{ background: '#FAEEDA', border: '0.5px solid #FAC775', borderRadius: 8, padding: '0.75rem 1rem', fontSize: 13, color: '#854F0B', marginBottom: '1rem', display: 'flex', gap: 8 }}>
              <span>⚠</span>
              <div>Il periodo di rinnovo {anno} è aperto fino al <strong>31 marzo {anno}</strong>. Completa il rinnovo per rimanere nella rete ADMA.</div>
            </div>
            ...
            <div style={{ background: 'white', border: '0.5px solid #e5e5e5', borderRadius: 12, padding: '1.5rem' }}>
              ...
              <div style={{ marginBottom: '1.25rem' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: '0.8rem' }}>
                  <div><label style={labelStyle}>Nome *</label><input style={inputStyle} value={form.nome_presidente} onChange={e => setForm({ ...form, nome_presidente: e.target.value })} placeholder="Nome" /></div>
                  <div><label style={labelStyle}>Cognome *</label><input style={inputStyle} value={form.cognome_presidente} onChange={e => setForm({ ...form, cognome_presidente: e.target.value })} placeholder="Cognome" /></div>
                </div>

                <div style={{ marginBottom: '0.5rem', fontSize: 12, color: '#555', fontWeight: 500 }}>Animatore spirituale locale</div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: '0.8rem' }}>
                  <div><label style={labelStyle}>Nome *</label><input style={inputStyle} value={form.nome_animatore} onChange={e => setForm({ ...form, nome_animatore: e.target.value })} placeholder="Nome" /></div>
                  <div><label style={labelStyle}>Cognome *</label><input style={inputStyle} value={form.cognome_animatore} onChange={e => setForm({ ...form, cognome_animatore: e.target.value })} placeholder="Cognome" /></div>
                </div>
        ...
        {sent && (
          <div style={{ background: '#EAF3DE', border: '0.5px solid #9FE1CB', borderRadius: 12, padding: '1.5rem', fontSize: 14, color: '#3B6D11' }}>
            <div style={{ fontWeight: 500, marginBottom: 6 }}>Rinnovo {anno} inviato con successo!</div>
            La richiesta è stata trasmessa alla Primaria di Valdocco. Una volta approvata riceverai il timbro {anno} sul tuo profilo.
